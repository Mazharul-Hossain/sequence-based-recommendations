...
class RNNOneHot(rnn.RNNBase):
    ...
    The parameters specific to the RNNOneHot are:
        diversity_bias: a float in [0, inf) that tunes how the cost function of the network is biased
        towards less seen movies.
            In practice, the classification error given by the categorical cross-entropy is divided
            by exp(diversity_bias * popularity (on a scale from 1 to 10)).
            This will reduce the error associated to movies with a lot of views, putting therefore more
            importance on the ability of the network to correctly predict the rare movies.
    ...
    def _prepare_networks(self, n_items):
        ...
        # The input is composed of to parts : the one-hot encoding of the movie, and the features of the movie
    ...
    def _prepare_input(self, sequences):
        ...
        for i, sequence in enumerate(sequences):
            ...
            seq_features = list(map(lambda x: self._get_features(x, user_id), in_seq))
