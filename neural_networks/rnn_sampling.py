...
class RNNSampling(rnn.RNNBase):
    ...
    def __init__(self, loss_function="Blackout", sampling=32, last_layer_tanh=False, last_layer_init=1.,
                 diversity_bias=0.0, sampling_bias=0., **kwargs):
        '''
        Parameters
        ----------
    ...
    def _prepare_networks(self, n_items):
        ...
        self.theano_inputs = [self.l_in.input_var, self.l_mask.input_var, target, samples, target_popularity,
                              self.exclude]

        # The sliced output is then passed through linear layer to obtain the right output size
        self.l_out = BlackoutLayer(l_last_slice, num_units=self.n_items, num_outputs=self.sampling, nonlinearity=None,
                                   W=lasagne.init.GlorotUniform(gain=self.last_layer_init))
