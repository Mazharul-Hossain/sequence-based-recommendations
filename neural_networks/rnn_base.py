...
class RNNBase(object):
    ...
    def train(self, dataset,
        ...
        max_time is used to set the maximum amount of time (in seconds) that the training can last before being stop.
            By default, max_time=np.inf, which means that the training will last until the training_set runs out,
            or the user interrupt the program.

        progress is used to set when progress information should be printed during training. It can be either
        an int or a float:
            integer : print at linear intervals specified by the value of progress (i.e. : progress,
            2*progress, 3*progress, ...)
            float : print at geometric intervals specified by the value of progress (i.e. : progress,
            progress^2, progress^3, ...)

        max_progress_interval can be used to have geometric intervals in the beginning then switch to linear intervals.
            It ensures, independently of the progress parameter, that progress is shown at least
            every max_progress_interval.

        time_based_progress is used to choose between using number of iterations or time as a progress indicator.
        True means time (in seconds) is used, False means number of iterations.

        auto-save is used to set whether the model should be saved during training. It can take several values:
        ...
        load_last_model: if true, find the latest model in the directory where models should be saved, and
        load it before starting training.

        early_stopping : should be a callable that will receive the list of validation error and the
        corresponding epochs and return a boolean indicating whether the learning should stop.
        """
            ...
            while time() - start_time < max_time and iterations < max_iter:
                ...
                if progress_indicator >= next_save:
                    ...
                    if progress_indicator >= min_iterations:
                        ...
                        run_nb = len(metrics[metrics_keys[0]]) - 1
                        if autosave == 'All':
    ...
    def _compute_validation_metrics(self, metrics):
        ...
        validation_data_set = self.dataset.validation_set(epochs=1)
        validation_mini_batch = self._gen_mini_batch(validation_data_set, test=True)
        try:
            for batch_input, goal in validation_mini_batch:
    ...
    def _gen_mini_batch(self, sequence_generator, test=False, max_reuse_sequence=np.inf):
        ...
        test determines how the sequence is split between training and testing
        ...
        if test == False, max_reuse_sequence determines how many time a single sequence is used in the same batch.
            with max_reuse_sequence = inf, one sequence will be used to make the whole batch (if the sequence
            is long enough)
        ...
        while True:
            ...
            while j < batch_size:
                ...
                if not test:
                    seq_lengths = sorted(random.sample(range(2, len(sequence)),
    ...
    def _get_ratings_features(self, item):
        ...
        ten, popularity on a log scale of ten]
        """
    ...
    def _get_features(self, item, user_id):
        """
        Change a tuple (item_id, rating) into a list of features to feed into the RNN features have the following
        structure: [one_hot_encoding, personal_rating on a scale of ten, average_rating on a scale of ten,
        popularity on a log scale of ten]
        """
