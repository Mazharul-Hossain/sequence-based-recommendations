...
class FISMCluster(RNNCluster):
    ...
    def _prepare_networks(self, n_items):
        ...
        self.l_out = BlackoutLayer(l_user_rep, num_units=self.n_items, num_outputs=self.n_samples, nonlinearity=None,
        ...
        if self.reg > 0.:
            self.cost += self.reg * lasagne.regularization.regularize_network_params(self.l_out,
        ...
        self.cluster_selection_layer = lasagne.layers.DenseLayer(l_user_rep, b=None, num_units=self.n_clusters,
                                                                 nonlinearity=None)
        ...
        self.cluster_repartition = theano.shared(
            (0.1 * np.random.randn(self.n_items, self.n_clusters)).astype(theano.config.floatX))
        if self.cluster_type == 'softmax':
    ...
    def _compile_train_function(self):
        ...
        self.train_function = theano.function(
            [self.l_in.input_var, self.target, self.samples, self.cluster_samples, self.exclude], self.cost,
            updates=updates, allow_input_downcast=True, name="Train_function", on_unused_input='ignore')
    ...
    def _compile_predict_function(self):
        ...
        if self.predict_with_clusters:
            ...
            theano_predict_function = theano.function([self.l_in.input_var], [user_representation, cluster_selection],
            ...
            user_representation = lasagne.layers.get_output(self.user_representation_layer, deterministic=True)
            theano_predict_function = theano.function([self.l_in.input_var], user_representation,
                                                      allow_input_downcast=True, name="Predict_function",
                                                      on_unused_input='ignore')
    ...
    def _compile_test_function(self):
        ...
        theano_test_function = theano.function(
            [self.l_in.input_var, self.target, self.samples, self.cluster_samples, self.exclude],
