...
def sequence_noise_command_parser(parser):
    ...
    parser.add_argument('--n_shuf_std',
                        help="The distance between the two items to be swapped is drawn from a normal distribution "
                             "whose std is defined by this parameter",
...
def get_sequence_noise(args):
    return SequenceNoise(dropout=args.n_dropout, swap=args.n_swap, ratings_perturb=args.n_ratings, shuf=args.n_shuf,
                         shuf_std=args.n_shuf_std)
...
class SequenceNoise(object):
    ...
    def __call__(self, sequence_generator):
        """Recieves a generator of sequences in the form ([(item, rating), (item, rating), ...], user) and generates
        sequences in the same format, after potentially applying dropout, item swapping and ratings modifications.
        """
