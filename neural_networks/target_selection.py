...
def target_selection_command_parser(parser):
    parser.add_argument('--n_targets', help='Number of targets (Only for RNN with hinge, logit or logsig loss).',
                        default=1, type=int)
    parser.add_argument('--shuffle_targets',
                        help='Instead of picking the next items in the sequence as the target(s), the targets are picked randomly in the remaining sequence.',
                        action='store_true')
    parser.add_argument('--rand_test_target',
                        help='Use the exact same procedure for target selection during training and testing. Otherwise shuffling and bias are used only during training.',
                        action='store_true')
    parser.add_argument('--target_bias',
                        help='Popular item are picked as item with a lower probability. Targets are skipped with a probability proportional to (number_of_views)^bias. Set negative bias to avoid this procedure.',
                        default=-1., type=float)


def get_target_selection(args):
    return SelectTargets(n_targets=args.n_targets, shuffle=args.shuffle_targets, bias=args.target_bias,
                         determinist_test=(not args.rand_test_target))
