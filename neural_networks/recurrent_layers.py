...
def recurrent_layers_command_parser(parser):
    parser.add_argument('--r_t', dest='recurrent_layer_type', choices=['LSTM', 'GRU', 'Vanilla'],
                        help='Type of recurrent layer', default='GRU')
    ...
    parser.add_argument('--r_emb',
                        help='Add an embedding layer before the RNN. Takes the size of the embedding as parameter, '
                             'a size<1 means no embedding layer.',
                        type=int, default=0)
...
class RecurrentLayers(object):
    ...
    def __call__(self, input_layer, mask_layer, true_input_size=None, only_return_final=True):
        ...
        if self.embedding_size > 0:
            ...
            l_emb = lasagne.layers.flatten(
                lasagne.layers.EmbeddingLayer(in_int32, input_size=true_input_size, output_size=self.embedding_size),
                outdim=3)
            l_rec = self.get_recurrent_layers(l_emb, mask_layer, true_input_size=None,
                                              only_return_final=only_return_final)
        else:
            l_rec = self.get_recurrent_layers(input_layer, mask_layer, true_input_size=true_input_size,
                                              only_return_final=only_return_final)
    ...
    def get_one_layer(self, input_layer, mask_layer, n_hidden, true_input_size, only_return_final):
        if self.bidirectional:
            forward = self.get_unidirectional_layer(input_layer, mask_layer, n_hidden, true_input_size,
                                                    only_return_final, backwards=False)
            backward = self.get_unidirectional_layer(input_layer, mask_layer, n_hidden, true_input_size,
                                                     only_return_final, backwards=True)
            return lasagne.layers.ConcatLayer([forward, backward], axis=-1)
        else:
            return self.get_unidirectional_layer(input_layer, mask_layer, n_hidden, true_input_size, only_return_final,
                                                 backwards=False)

    def get_unidirectional_layer(self, input_layer, mask_layer, n_hidden, true_input_size, only_return_final,
                                 backwards=False):
