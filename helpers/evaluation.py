...
class Evaluator(object):
    ...
    It is used by first adding a series of "instances" : pairs of goals and predictions, then metrics can be
    computed on the ensemble of instances:
    ...
    def average_precision(self):
        ...
        for goal, prediction in self.instances:
            if len(prediction) > 0:
                precision += float(len(set(goal) & set(prediction[:min(len(prediction), self.k)]))) / min(
                    len(prediction), self.k)
